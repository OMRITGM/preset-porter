...
if (!model || !files.length) {
  console.error(`Usage: preset-porter --to <printer> [--reset-cooling] [-o outDir] <preset.json> [...]\n       preset-porter --list\n(printer map built from Bambu Studio ${studioVersion})`);
  process.exit(2);
}
...
for (const f of files) {
  try {
    const r = portPreset(JSON.parse(fs.readFileSync(f, "utf8")), model, { resetMachine });
