...
function portPreset(src, model, MAP = BBL_MAP) {
  const notes = [], out = JSON.parse(JSON.stringify(src));
  ...
  if (parent && !base) notes.push(`Parent "${parent}" is not a Bambu system preset (custom or third-party parent) - falling back to Generic.`);
  ...
  if (parent && !leaf) {
    ...
    if (base) notes.push(`"${base.replace(" @base", "")}" has no ${short} version - inherited from "${leaf}" instead. Check speeds and cooling.`);
  }
  if (parent) { out.inherits = leaf; delete out.compatible_printers; delete out.compatible_printers_condition; }
    ...
    notes.push("Preset has no parent (flattened export) - retargeted its printer list; values were kept as-is.");
  }
  ...
  if (added.length && varKeys.length) notes.push(`${short} has nozzle variants the source never defined (${added.join(", ")}) - those inherit the system values.`);
  ...
  return { preset: out, notes, parent, newParent: out.inherits || null };
}
